...
@if (!empty($timeSlots))
    ...
    @foreach ($timeSlots as $slot)
        <label class="btn btn-light col-sm-3 @if ($slot['is_available'] == 1) blocked @endif">
            <input type="checkbox" class="hidden-checkbox slot-checkbox @if($slot['is_available'] == 1) not-available @endif" name="slots[]" id="slot_number_{{ $i }}"
                value="{{ $slot['start'] }} - {{ $slot['end'] }}" @if ($slot['is_available'] == 1) disabled readonly @endif>
            @php
...
@else
    <label class="btn btn-light col-sm-12">
        <div>{{ __('labels.Today Barber Not Available..') }}</div>
    </label>
@endif
    ...
    $(document).ready(function() {
        ...
        $(".slot-checkbox").click(function(e) {
            ...
            if (selectedServiceCount > 0) {
                ...
                $(".slot-checkbox").each(function(index) {
                    if (index >= startIndex && selectedSlots < selectedServiceCount) {
                        if ($(this).hasClass('not-available')) {
                            toastr.error("Need to select another slot because the next slot is already booked.");
                            clearSlots();
                ...
                toastr.error("Select a service first.");
            }
        });
    });
</script> --}}
    ...
    $(".slot-checkbox").click(function(e) {
        ...
        if (selectedServiceCount > 0) {
            ...
            if (availableSlots < selectedServiceCount) {
                toastr.error("{{ __('error.Not enough available slots. Please select another slot.') }}");
                clearSlots();
                return false;
            }
            ...
            $(".slot-checkbox").each(function(index) {
                if (index >= startIndex && selectedSlots < selectedServiceCount) {
                    if ($(this).hasClass('not-available')) {
                        toastr.error("{{ __('error.Need to select another slot because the next slot is already booked.') }}");
                        clearSlots();
                        validSelection = false;
                        return false; // Exit the loop and function
                    }
