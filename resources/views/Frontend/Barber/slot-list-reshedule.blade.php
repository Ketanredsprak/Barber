...
@if (!empty($timeSlots))
    ...
    @foreach ($timeSlots as $slot)
        <label class="btn btn-light col-sm-3 @if ($slot['is_available'] == 1) blocked @endif">
            ...
            @endphp
            <div>{{ $start }} - {{ $end }}</div>
            <div class="checked-img d-none"><img src="{{ static_asset('frontend/assets/images/check_mark.webp') }}"
                    alt="image" height="20px" width="20px"></div>
        </label>
    @endforeach
@else
            ...
            if (selectedServiceCount > 0) {
                ...
                $(".slot-checkbox").each(function(index) {
                    if (index >= startIndex && selectedSlots < selectedServiceCount) {
                        ...
                        $(this).siblings('.checked-img').removeClass(
