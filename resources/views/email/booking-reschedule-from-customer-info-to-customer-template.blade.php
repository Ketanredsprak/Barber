              ...
              <table width="560" border="0" cellspacing="0" cellpadding="0" align="center" class="templetcontainer">
                ...
                <tr>
                  <td align="left" valign="top"
                    ...
                    <h2>Your Appointment Resechedule Succesfully..</h2>
                    <p>Hi [{{ @$old_booking_data->customer_detail->first_name ?? "" }} {{ @$old_booking_data->customer_detail->last_name ?? "" }}], your appointment with [{{ @$old_booking_data->barber_detail->first_name ?? "" }} {{ @$old_booking_data->barber_detail->last_name ?? "" }}] at [{{ @$old_booking_data->barber_detail->first_name ?? "" }} {{ @$old_booking_data->barber_detail->last_name ?? "" }}] on [{{ @$old_booking_data->booking_date ?? "" }} {{ @$old_booking_data->start_time ?? "" }} - {{ @$old_booking_data->end_time ?? "" }}] is now resechedule on [{{ @$new_booking_data->booking_date ?? "" }} {{ @$new_booking_data->start_time ?? "" }} - {{ @$new_booking_data->end_time ?? "" }}]. and our system also notified your [{{ @$old_booking_data->barber_detail->first_name ?? "" }} {{ @$old_booking_data->barber_detail->last_name ?? "" }}] for resechedule appointment.</p>
                    <h3>New Appointment Detail</h3>
                    <p>Salon Name [{{ @$new_booking_data->barber_detail->salon_name ?? "" }}] </p>
                    <p>Appointment Date [{{ @$new_booking_data->booking_date ?? "" }}] </p>
                    <p>Appointment Start and End Time [ {{ @$new_booking_data->start_time ?? "" }} -  {{ @$new_booking_data->end_time ?? "" }}] </p>
                </td>
                </tr>
                <tr>
                  <td height="40">&nbsp;</td>
